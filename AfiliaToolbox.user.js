...
(function () {
    ...
    const HOSPITAL_BEDS_KEY = 'hospitalBedsCache';
    const HOSPITAL_STATIONS_URL =
        '/api/game/stations/getAllStations';
    ...
    let gameSessionID = '';
    let vehicleDataGameID = '';
    let vehicleDataSettlePromise = null;
    ...
    async function loadData() {
        ...
        const resolvedGameID =
            resolveGameSessionID();

        if (resolvedGameID) {
            await settleVehicleDataForGame(
                resolvedGameID
            );

            await settleHospitalDataForGame(
                resolvedGameID
            );
        }
    ...
    function getCachedGameBlock(cached, gameId) {
        if (
            !cached ||
        ...
        return cached.games[gameId] || null;
    }

    async function settleVehicleDataForGame(gameId) {
        if (
            !gameId ||
            vehicleDataGameID === gameId
        ) {
            return false;
        }

        if (vehicleDataSettlePromise) {
            return vehicleDataSettlePromise;
        }

        vehicleDataSettlePromise = (async () => {
            vehicleCatalog.clear();
            vehicleCatalogByCallsign.clear();
            vehicleDistance.clear();

            vehicleDataGameID = gameId;

            const [catalogCache, distanceCache] =
                await Promise.all([
                    dbGet(VEHICLE_CATALOG_KEY),
                    dbGet(VEHICLE_DISTANCE_KEY)
                ]);
    ...
    async function saveVehicleData() {
        ...
        const catalogGames =
            catalogCache && catalogCache.games
                ? catalogCache.games
                : {};
        ...
        const distanceGames =
            distanceCache && distanceCache.games
                ? distanceCache.games
                : {};

        distanceGames[gameId] = {
    ...
    function trackGameSessionIDFromURL(url) {
        ...
        settleVehicleDataForGame(gameSessionID)
            .then(settled => {
                if (settled) {
        ...
        settleHospitalDataForGame(gameSessionID)
            .catch(() => {});
    }
        ...
        if (typeof originalFetch === 'function') {
            window.fetch = function (
                ...
                if (
                    ...
                    url.includes(HOSPITAL_SESSION_URL)
                ) {
                    request.then(response => {
                        if (
                            response &&
    ...
    function getHospitalBedSummary() {
        ...
        for (const entry of hospitalBeds.values()) {
            if (
                !entry ||
    ...
    async function settleHospitalDataForGame(gameId) {
        if (
            !gameId ||
            hospitalDataGameID === gameId
        ) {
            return false;
        }

        if (hospitalDataSettlePromise) {
            return hospitalDataSettlePromise;
        }

        hospitalDataSettlePromise = (async () => {
            hospitalBeds.clear();

            hospitalDataGameID = gameId;

            const cached =
                await dbGet(HOSPITAL_BEDS_KEY);

            const block =
                getCachedGameBlock(cached, gameId);
            ...
            renderHospitalBedsPill();

            return true;
        })().finally(() => {
            hospitalDataSettlePromise = null;
        });

        return hospitalDataSettlePromise;
    }

    function scheduleHospitalSave() {
        if (hospitalSaveTimer) {
            clearTimeout(hospitalSaveTimer);
        }

        hospitalSaveTimer = setTimeout(() => {
            hospitalSaveTimer = null;

            saveHospitalData().catch(console.error);
        }, HOSPITAL_SAVE_DELAY);
    }
    ...
    async function saveHospitalData() {
        ...
        for (const [id, entry] of hospitalBeds) {
            entries[id] = {
                used: entry.used,
                total: entry.total,
                fetchedAt: entry.fetchedAt
    ...
    function extractStationsFromPayload(payload) {
        if (Array.isArray(payload)) {
            if (
                payload.length === 0 ||
                isStationLike(payload[0])
            ) {
                return payload;
            }
    ...
    function handleHospitalStationsPayload(payload) {
        if (!gameSessionID) {
            return;
        }

        const stations =
            extractStationsFromPayload(payload);
        ...
        for (const station of stations) {
            if (
                ...
                !station.config ||
                !station.config.patient_slots ||
                typeof station.config.patient_slots.total !==
                    'number'
            ) {
                continue;
            }
            ...
            const used =
                typeof station.config.patient_slots
                    .used === 'number'
                    ? station.config.patient_slots.used
                    : 0;

            const total =
                station.config.patient_slots.total;

            const existing =
                hospitalBeds.get(station.id);
        ...
        if (changed) {
            scheduleHospitalSave();
        }

        renderHospitalBedsPill();
    }

    function findStatusBarColumn() {
        const candidates = Array.from(
    ...
    function renderHospitalBedsPill() {
        const column =
            findStatusBarColumn();
        ...
        const summary =
            getHospitalBedSummary();

        if (summary.total <= 0) {
            const existing =
                document.getElementById(
                    'afilia-hospital-beds-pill'
                );
        ...
        const percentage = Math.round(
            (summary.used / summary.total) * 100
        );

        const freeBeds =
            summary.total - summary.used;
        ...
        if (!pill) {
            ...
            pill.className =
                'afilia-hospital-beds-pill flex items-center gap-1.5 sm:gap-2 bg-gray-900/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md';

            pill.innerHTML =
                '<i class="fa-solid fa-bed-pulse text-blue-400 text-xs sm:text-base"></i>' +
                '<div class="flex flex-col leading-none sm:leading-tight">' +
                '<span class="text-[9px] sm:text-[10px] text-gray-400">Bettenauslastung</span>' +
                '<span class="afilia-hospital-beds-value font-semibold ' +
                statusClass +
                '">' +
                summary.used +
                '/' + summary.total +
            ...
            if (value) {
                ...
                value.className =
                    'afilia-hospital-beds-value font-semibold ' +
                    statusClass;
            }
        }
